...
const addRoutine = (req, res, db) => {
    ...
    if (!image || !teacherClass) 
      return res.status(400).send({ success: false, message: 'Missing image or teacherClass' });
  
    db.query(
      'UPDATE teachers SET routine = ? WHERE teacherClass = ?', 
      [image.buffer, teacherClass], 
      (err) => err ? res.status(500).send({ success: false, message: 'Database error' }) : res.send({ success: true, message: 'Routine added successfully!' })
    );
  };
